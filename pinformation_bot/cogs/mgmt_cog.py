...
class ManagementCog(commands.Cog, name="Main"):
    ...
    async def info_embed(self, ctx: commands.Context[PinformationBot]):  # unprived command
        """
        ...
        _ = await ctx.reply(embed=embed, mention_author=False, ephemeral=True)

    @commands.hybrid_command(name="manageadmin")
    @commands.check(check_admin)
    async def manage_admin(
        self, ctx: commands.Context[PinformationBot], action: Literal["add", "remove"], user: discord.User
    ):
        """
        ...
        if action == "add":
            if user_id in self.bot.config.admin_users:
                _ = await ctx.reply("User already in admin list!", ephemeral=True)
                return
            ...
            _ = await ctx.reply(msg, ephemeral=True)
        if action == "remove":
            if user_id not in self.bot.config.admin_users:
                _ = await ctx.reply("User not in admin list!", ephemeral=True)
                return
    ...
    async def manage_admin_role(
        self, ctx: commands.Context[PinformationBot], action: Literal["add", "remove"], role: discord.Role
    ):
        """
        ...
        if action == "add":
            if role_id in self.bot.config.admin_roles:
                _ = await ctx.reply("Role already in admin list!", ephemeral=True)
                return
        ...
        if action == "remove":
            if role_id not in self.bot.config.admin_roles:
                _ = await ctx.reply("Role not in admin list!", ephemeral=True)
                return
    ...
    async def manage_role(
        ...
        if action == "add":
            ...
            if channel_id in permitted_roles[role_id]:
                _ = await ctx.reply("Role already has access to this channel!", ephemeral=True)
                return
            ...
            _ = await ctx.reply(f"Added {channel.mention} to {role.name}'s permitted list", ephemeral=True)
        if action == "remove":
            if role_id not in permitted_roles or channel_id not in permitted_roles[role_id]:
                _ = await ctx.reply("Role or channel not in permitted list!", ephemeral=True)
                return
    ...
    @commands.check(check_admin)
    async def set_log_channel(self, ctx: commands.Context[PinformationBot], channel: discord.TextChannel):
        """
    ...
    async def log_mgmt_change(self, ctx: commands.Context[PinformationBot], cmd_msg: str) -> None:
