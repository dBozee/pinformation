...
To run the bot locally, you will need to have [UV](https://docs.astral.sh/uv/getting-started/installation/) installed.
...
3. deploy the bot by running `uv run python -m pinformation_bot`
4. verify the bot is running by checking in discord. The bot should now show online. You can try running `/botinfo` and
   should get an ephemeral embed response showing info on the bot.
...
- Create a `.env` file and in it place your Discord API token with `DISCORD_TOKEN=YOUR_TOKEN_HERE`

#### Starting the docker container

1. Navigate to the root /pinformation/ directory
2. Configure the bot by modifying the `config/config.json` file. You'll need to add at least your
   own discord ID or role ID to the list of permitted users before deploying the bot. Additional users/roles can be
   added using slash commands later.
    - **Reminder:** the IDs have to be stored as strings **NOT** ints. This is a JSON limitation with int sizes.

3. run `docker-compose up -d` and wait for the compose command to complete.
4. verify the bot is running by checking in discord. The bot should now show online. You can try running `/botinfo` and
   should get an ephemeral embed response showing info on the bot.
...
The only management command which doesn't require management permissions is `/botinfo`
...
- **shutdown**
    - Shut down the bot.
    - Management perms required
    - **WARNING!** - Using this command(currently) in the docker deployment will require you to start the container
      again.
